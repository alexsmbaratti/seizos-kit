    ...
    private func formatTermsContent(_ content: String) {
        ...
        if let headingPattern = headingPattern,
            let contentRange = Range(
                NSRange(location: 0, length: content.utf16.count),
                in: content
            )
        {

            let matches = headingPattern.matches(
                in: content,
                range: NSRange(contentRange, in: content)
            )
...
#Preview {
    let termsOfUse = """
        ...
        """
    let lastUpdatedText =
        "Last Updated: \(Calendar.current.date(from: DateComponents(year: 1970, month: 1, day: 1))!.formattedLongDate())"
